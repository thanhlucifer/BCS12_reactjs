...
const RenderShose = () => {
    ...
    function render() {
        return arrShose.map((item, index) => (
            <div className='render-shose-item space-y-5' key={index}>
                <img src={item.image} alt="" />
                <h3 className='font-bold text-xl text-title'>{item.name}</h3>
                <p className='text-lg font-medium text-gray-500'>{item.price}</p>
                <button className='py-2 px-5 bg-red-500 text-white rounded-lg'>Xem ngay</button>
            </div>
        ));
    }
